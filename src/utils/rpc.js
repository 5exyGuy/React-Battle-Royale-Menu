...
class RPCBrowser { 
   ...
   async callServer(procedureName, params = {}) {
      let promiseEvent = new Promise((resolve, reject) => {
         ...
         window.alt.emit("rpc::browser::callServer", {__rpcPendingUid: uid, __rpcListenersName: procedureName }, params);

         window.alt.on('rpc::browser::callServerResponse', (info, result) => {
   ...
   async callClient(procedureName, params = {}) {
      let promiseEvent = new Promise((resolve, reject) => {
         ...
         window.alt.emit("rpc::browser::callClient", {__rpcPendingUid: uid, __rpcListenersName: procedureName }, params);

         window.alt.on('rpc::browser::callClientResponse', (info, result) => {
            if(window.__rpcPending["__rpcPending-" + uid]) {
               let resolver = info && info.error ? window.__rpcPending["__rpcPending-" + uid].reject : window.__rpcPending["__rpcPending-" + uid].resolve;
               window.__rpcPending["__rpcPending-" + uid] = undefined;  
               return resolver(result);
   ...
   responseListener() {
      ...
      window.alt.on('rpc::browser::callBrowser', (info, params) => {
         let result;
         ...
         window.alt.emit("rpc::browser::callBrowserResponse", info, result);
      });
   }
